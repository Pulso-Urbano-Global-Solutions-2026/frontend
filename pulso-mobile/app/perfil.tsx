...
export default function PerfilScreen() {
  ...
  if (loadingData) return <SafeAreaView style={styles.safe}><LoadingSkeleton rows={5} style={styles.skeleton} /></SafeAreaView>;
  if (error) return <SafeAreaView style={styles.safe}><ErrorState message={error} onRetry={() => { setError(null); setLoadingData(true); }} /></SafeAreaView>;
  ...
  return (
    <SafeAreaView style={styles.safe}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <TextInput style={styles.input} value={nome} onChangeText={setNome} placeholder="Nome" placeholderTextColor={Colors.textDim} />
        ...
        <TouchableOpacity style={styles.saveBtn} onPress={handleSave} disabled={saving}>
          {saving ? <ActivityIndicator color={Colors.bg} /> : <Text style={styles.saveBtnText}>Salvar</Text>}
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  input: { backgroundColor: Colors.surface, borderColor: Colors.border, borderWidth: 1, borderRadius: 8, padding: 14, color: Colors.text, fontSize: Typography.size.md },
  ...
  saveBtn: { backgroundColor: Colors.bom, borderRadius: 8, padding: 14, alignItems: 'center', marginTop: 8 },
  ...
  danger: { borderWidth: 1, borderColor: Colors.critico, borderRadius: 12, padding: 16, marginTop: 16 },
